...
//                            ProcessTaskStatus.FAILED,
         ...
         * 步骤撤回权限逻辑在{@link neatlogic.module.process.operationauth.handler.StepOperateHandler#init}中的ProcessTaskOperationType.STEP_RETREAT里
         */
...
//                    }
         ...
         * 步骤转交权限逻辑在{@link neatlogic.module.process.operationauth.handler.StepOperateHandler#init}中的ProcessTaskOperationType.STEP_TRANSFER里
         */
        ...
        operationBiPredicateMap.put(ProcessTaskOperationType.PROCESSTASK_FOCUSUSER_UPDATE,
                (processTaskVo, processTaskStepVo, userUuid, operationTypePermissionDeniedExceptionMap, extraParam) -> {
                    ...
                    //3.依次判断当前用户是否是工单上报人、代报人、处理人、待处理人，如果都不是，则提示“您不是工单干系人”；
                    if (userUuid.equals(processTaskVo.getOwner())) {
                        return PredicateResult.ACCEPT;
                    } else if (userUuid.equals(processTaskVo.getReporter())) {
                        return PredicateResult.ACCEPT;
