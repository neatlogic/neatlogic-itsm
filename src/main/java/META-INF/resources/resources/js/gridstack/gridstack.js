...
(function (factory) {
    ...
    GridStackEngine.prototype.set_height = function (sender,tempGsHeight,tempHeight,node) {
    	if(tempGsHeight){
    		sender.attr("data-gs-height",tempGsHeight);// 改变界面中node节点的height
    	}else{
    		tempGsHeight = sender.attr("data-gs-height");
    	}
    	if(sender.attr('controltype')== 'mselect'){
    		sender.find("select").height('auto');// 拖动后初始化多选下拉框高度
    		var myselectH = sender.find("select").outerHeight();
    		tempHeight = myselectH+40;
    	}else if(sender.attr('controltype')== 'textarea'){
    		sender.find("textarea").parent().height(tempHeight-44);
    		sender.find("textarea").height(tempHeight-50);// 拖动后初始化textarea高度
    	}else if( (sender.attr('controltype')== 'mradio') || (sender.attr('controltype') == 'mcheckbox')){
    		var myselectH = sender.find(".tal").outerHeight();
    		tempHeight = myselectH+40;    
    	}
	    if(sender.data('_gridstack_node')){
			sender.data('_gridstack_node').height = Math.max(parseInt(tempGsHeight),parseInt(tempHeight));// 改变data中node节点的height
		}else{
			if(node){
				node.height = Math.max(parseInt(tempGsHeight),parseInt(tempHeight));// 改变data中node节点的height
			}else{
				sender.height(Math.max(parseInt(tempGsHeight),parseInt(tempHeight)));
				sender.attr("data-gs-height",Math.max(parseInt(tempGsHeight),parseInt(tempHeight)));
			}
		}
    ...
    GridStackEngine.prototype.add_node = function(node) {
        ...
        if(typeof node.auto_position_tag == 'undefined'){
        	node.auto_position_tag = false;
        }
        ...
        if (node.auto_position&& node.auto_position_tag) {// 自动布局
            this._sort_nodes();
        ...
        var temp = $(n.el).attr("data-gs-y");
        var tempGsHeight;
        var $this = this;
        $("div[data-gs-y='"+temp+"']").not(".grid-stack-placeholder").not(n.el).each(function(){
        	tempGsHeight = $(this).attr("data-gs-height");
        })
        if(tempGsHeight){//同行第二个
	        var tempHeight = tempGsHeight*n.cell_height-n.gap;
	        $("div[data-gs-y='"+temp+"']").not(".grid-stack-placeholder").each(function(){
	        	$this.set_height($(this),tempGsHeight,tempHeight,node);
	        })
        }else{
        	$("div[data-gs-y='"+temp+"']").not(".grid-stack-placeholder").each(function(){//同行第一个
        		var tempHeight = $(this).attr("data-gs-height")*n.cell_height-n.gap;
        		$this.set_height($(this),null,tempHeight,null);
	        })
        }
        /************************************************************************************ */
        return node;
    ...
    var GridStack = function (el, opts) {
        ...
        var is_nested = this.container.closest('.' + opts.item_class).size() > 0; // 是否存在grid-stack-item

        this.opts = _.defaults(opts || {}, {
            width: parseInt(this.container.attr('data-gs-width')) || 12,
            height: parseInt(this.container.attr('data-gs-height')) || 0,
            ispre:false,// 是否为预览页，是为true(addBy kong)
            auto_position_tag:true,// 标签是否自动布局(addBy kong)
            item_class: 'grid-stack-item',
            placeholder_class: 'grid-stack-placeholder',
            handle: '.grid-stack-item-content',
            cell_height: 1, // 容器高度
            min_cell_width: 2, // 最小容器宽度(addBy kong)
            min_cell_height:70,// 最小高度 ：cell_height*min_cell_height
            vertical_margin: 0,// 上下间距距离
            vertical_tb:4,// 上下间距距离(addBy kong)
            vertical_rl: 4, // 左右间距距离(addBy kong)
            gap:5,//textarea、mutiselect 到底部距离
            auto: true,
            min_width:100,
            float: false,
        ...
        var formHtml = this.container.html();
        // 避免初始化布局错误
        formHtml = formHtml.replace(/<div\sclass=['"]grid-stack-placeholder\sgrid-stack-item['"]\s.*?>[\s\S]*?<div class=['"]placeholder-content['"]>[\s\S]*?<\/div>[\s\S]*?<\/div>/g,""); 
		// 避免初始化不能拉伸标签
        formHtml = formHtml.replace(/<div\sclass=['"]ui-resizable-handle\sui-resizable-se\sui-icon\sui-icon-gripsmall-diagonal-se['"]\sstyle=['"].*?['"]>[\s\S]*?<\/div>/g,"");
        this.container.html(formHtml);
        this.container.addClass(this.opts._class);// 给对象添加grid-stack-??? class
        
        if (is_nested) {
            this.container.addClass('grid-stack-nested');// 存在"grid-stack-item"
															// class，就添加"grid-stack-nested"
															// class
        }

        this._init_styles();// 在<header>初始化<style></style>
        ...
        if (this.opts.auto) {
            ...
            this.container.children('.' + this.opts.item_class).each(function (index, el) {
                ...
                elements.push({
                    el: el,
                    i: parseInt(el.attr('data-gs-x')) + parseInt(el.attr('data-gs-y')) * _this.opts.width // Use
																											// opts.width
																											// as
																											// weight
																											// for
        ...
        this.placeholder = $('<div class="' + this.opts.placeholder_class + ' ' + this.opts.item_class + '"><div class="placeholder-content" /></div>').hide();// 添加移动时的虚线框
    ...
    GridStack.prototype._update_styles = function (max_height) {
        ...
        if (max_height > this._styles._max) {  // 建立规则,比如data-gs-y="1" 高度是60px
            for (var i = this._styles._max; i < max_height; ++i) {
                ...
                // 设置textarea 高度
                Utils.insert_css_rule(this._styles,
                        prefix + '[data-gs-y="' + i + '"] textarea',
                        'top: ' + (this.opts.cell_height * i + this.opts.vertical_margin * i)-10 + 'px;',
                        i
                 );
            }
    ...
    GridStack.prototype._prepare_element = function (el) {
        ...
        var node = self.grid.add_node({// 设置属性
            x: el.attr('data-gs-x'),
            y: el.attr('data-gs-y'),
            ...
            min_height: this.opts.min_cell_height,// el.attr('data-gs-min-height'),
            ...
            el: el,
            ispre:this.opts.ispre,
            auto_position_tag:this.opts.auto_position_tag,
            min_cell_width:this.opts.min_cell_width,
            cell_height: this.opts.cell_height,
            gap:this.opts.gap
        });
        ...
        var on_end_moving = function (event, ui) {
            ...
            /** *******************************同行高度一样*************************************************** */
            var n =node;
            var tempHeight = n.height*n.cell_height-n.gap;
            var temp = $(n.el).attr("data-gs-y");
            var tempGsHeight = $(n.el).attr("data-gs-height");
            $("div[data-gs-y='"+temp+"']").not(".grid-stack-placeholder").each(function(){
            	/*$(this).attr("data-gs-height",tempGsHeight);// 改变界面中node节点的height
            	$(this).find("textarea").height(tempHeight);// 拖动后初始化textarea高度
            	$(this).find("textarea").css("width","100%");
            	if($(this).find("select").attr("multiple")){
	        		$(this).find("select").height(tempHeight);// 拖动后初始化多选下拉框高度
	        	}
            	var nodeTemp = $(this).data('_gridstack_node');
            	nodeTemp.height = parseInt(tempGsHeight);// 改变data中node节点的height
*/            	//console.log(nodeTemp)
            	self.grid.set_height($(this),tempGsHeight,tempHeight,null);
            	self.grid._fix_collisions($(this).data('_gridstack_node'));//-重新布局
            	self.grid._pack_nodes();//- 
            	self.grid._notify();//-
            })
            //var tempArray = new Array()
             $("div[data-gs-y]").not(".grid-stack-placeholder").not("div[data-gs-y='"+temp+"']").each(function(){
            	var temp = $(this).attr("data-gs-y");
            	/*if($.inArray(temp, tempArray)==-1){
            		tempArray.push(temp);
            	...
            	var maxHeight = 0;
            	$("div[data-gs-y='"+temp+"']").not(".grid-stack-placeholder").each(function(){
            		var tempGsHeight = $(this).attr('data-gs-height');
            		if(maxGsHeight < tempGsHeight) {
            			maxGsHeight = tempGsHeight;
            			maxHeight = maxGsHeight*n.cell_height-n.gap;
            		}
            	})
            	$("div[data-gs-y='"+temp+"']").not(".grid-stack-placeholder").each(function(){
            	/*	$(this).attr("data-gs-height",maxGsHeight);// 改变界面中node节点的height
            		$(this).find("textarea").height(maxHeight);// 拖动后初始化textarea高度
            		$(this).find("textarea").css("width","100%");
                 	if($(this).find("select").attr("multiple")){
     	        		$(this).find("select").height(maxHeight);// 拖动后初始化多选下拉框高度
     	        	}
                 	var nodeTemp = $(this).data('_gridstack_node');
                 	nodeTemp.height = parseInt(maxGsHeight);// 改变data中node节点的height
*/                 	self.grid.set_height($(this),maxGsHeight,maxHeight,null);
            		self.grid._fix_collisions($(this).data('_gridstack_node'));//-重新布局
                 	self.grid._pack_nodes();//- 
                 	self.grid._notify();//-
            	})
        ...
        if(!this.opts.ispre){// 预览页不需要拖拽
	        el.draggable(_.extend(this.opts.draggable, {
	            ...
	            resize: function (event, ui) {
	                ...
	                if(width<node.min_cell_width){// 如果小于最小宽度，则宽度为最小宽度
	                	width = node.min_cell_width;
	                }
        ...
        var left_position = ''+this.opts.vertical_rl+'px';
        var top_position = ''+this.opts.vertical_tb+'px';
        this.container.find('.grid-stack-item-content').css({"left":left_position,"right":left_position,"top":top_position,"bottom":top_position});// 设置便签之间的间距
    };
    ...
    GridStack.prototype.add_widget = function (el, x, y, width, height, auto_position,auto_position_tag) {
        el = $(el);
        ...
        if(auto_position_tag){ // 自动布局
        	this.opts.auto_position_tag = true;
        }
