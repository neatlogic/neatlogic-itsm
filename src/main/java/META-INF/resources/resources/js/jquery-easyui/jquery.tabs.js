...
(function($) {
	...
	function createTab(container, pp, options) {
		...
		opts.tab = $('<li closable="'+ opts.closable +'" class="tabTitle"' + (options.id ? ' index="' + options.id + '" id="tabTitle' + options.id + '"' : '') + '></li>').appendTo(tabs); 
		/*opts.tab.on('contextmenu',function(){
			return false;
		});
		opts.tab.on('mousedown', function(e) {
			var $tab = $(this);
			if (3 == e.which) {
				e.preventDefault();
				e.stopPropagation();
				var $menu = $('#tabMenu');
				if ($menu.length <= 0) {
					var $menu = $('<ul id="tabMenu" class="dropdown-menu pull-right" style="position:absolute;width:150px;text-align: left" role="menu"></ul>');
					$('body').append($menu);
				}
				
				if(!$menu.is(':visible')){
					$menu.data('index', $tab.attr('index'));
					var top = $tab.offset().top + $tab.outerHeight(true);
					...
					if($tab.attr('closable') == 'true'){
						var $li = $('<li><a href="javascript:void(0);">关闭当前标签页</a></li>');
						$menu.append($li);
					...
					$menu.slideDown('fast', function(){$menu.html('');});
				}else{
					if($menu.data('index') == $tab.attr('index')){
						$menu.slideUp('fast');
					}else{
						$menu.hide();
						$menu.data('index', $tab.attr('index'));
