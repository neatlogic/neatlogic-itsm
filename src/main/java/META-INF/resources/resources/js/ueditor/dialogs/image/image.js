...
(function () {
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            // WebUploader实例
                uploader,
                actionUrl = editor.getOpt('imageUrl'),//editor.getActionUrl(editor.getOpt('imageActionName')),
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    if (json.state == 'SUCCESS') {
                        ...
                        //edit by chenqiwei
                    	var form = $(window.parent.document).find('#' + editor.key).parents('form:first');
                    	if(form.length > 0){
                    		form.append('<input type="hidden" value="'+ json.fileid +'" name="fileId" class="hidUploadFileId">');
                    		try{
                    			window.parent.fileUploaded({fileid:json.fileid, extension:json.extension, size:json.size, belong:json.belong, name:json.original});
                    		}catch(e){
                    			console.log(e);
                    		}
                   		}
                        ...
                        $file.find('.error').attr('title', json.state).text(json.state).show();
                    }
                } catch (e) {
                    $file.find('.error').attr('title', lang.errorServerUpload).text(lang.errorServerUpload).show();
                }
            });
