...
$.widget("ui.draggable", $.ui.mouse, {
	...
	_mouseDrag: function(event, noPropagation) {
		...
		var st = 0;
		var sl = 0;
		/*if(this.helper.position().top != this.helper.offset().top){   //如果被拖动对象position().top不等于.offset().top，则它的top需加上scroll的top或left
			st = this.scrollParent.scrollTop();
		}
		if(this.helper.position().left != this.helper.offset().left){
			sl = this.scrollParent.scrollLeft();
		}*/
		this.position.left = this.position.left + sl;
		this.position.top = this.position.top +st;
		this.helper[ 0 ].style.left = this.position.left  + "px";
		this.helper[ 0 ].style.top = this.position.top  + "px";
		//console.log("ccccc    "+this.helper[ 0 ].style.top)
		if ($.ui.ddmanager) {
			$.ui.ddmanager.drag(this, event);
			//console.log("ccccc    "+this.helper[ 0 ].style.top);
			//console.log("ttttt    "+ this.position.top );
		}
	...
	_generatePosition: function( event, constrainPosition ) {
		...
		var st = 0;
		var sl = 0;
		if(parseInt(this.helper.position().top) != (parseInt(this.helper.offset().top)-5)){   //如果被拖动对象position().top不等于.offset().top，则它的top需加上scroll的top或left
			st = this.scrollParent.scrollTop();
		}
		if(this.helper.position().left != this.helper.offset().left){
			sl = this.scrollParent.scrollLeft();
		}
		//console.log("this.helper.position().top "+this.helper.position().top);
		//console.log("this.helper.offset().top "+this.helper.offset().top);
		//console.log("this.helper.position().left " +this.helper.offset().left);
		//console.log("this.helper.position().left " +this.helper.offset().left);
		/*console.log("this.offset.click.top "+this.offset.click.top);
		console.log("this.offset.relative.top "+this.offset.relative.top);
		console.log("this.offset.parent.top "+this.offset.parent.top);*/
		//console.log("this.cssPosition === 'fixed' "+(this.cssPosition) );
		return {
			top: (
				pageY -																	// The absolute mouse position
				this.offset.click.top	-												// Click offset (relative to the element)
				this.offset.relative.top -												// Only for relative positioned nodes: Relative offset from element to offset parent
				this.offset.parent.top + st +												// The offsetParent's offset without borders (offset + border)
			...
			left: (
				...
				this.offset.parent.left + sl +												// The offsetParent's offset without borders (offset + border)
...
$.widget("ui.resizable", $.ui.mouse, {
	...
	_mouseDrag: function(event) {//拉大缩小
		var data, props,
