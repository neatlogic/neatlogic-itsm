...
editor.backend.Backend = Class.extend({
    ...
    create: function(flowName,content,bg,chartType, successCallback, errorCallback){
        $.post("/asmconsole/flow/addNewFlow.do",  {
            chartType: chartType,
            bg: bg,
            flowName: flowName,
    ...
    del: function(definitionId, successCallback, errorCallback){
        $.post("/asmconsole/flow/delFlowByID.do",  {
            flowId: definitionId
        }).done(function(data) {
                successCallback(data);
            });
