	...
	$.fn.iframeAutoHeight = function(spec) {
		...
		return this.each(function() {
			...
			strategies['default'] = function(iframe, $iframeBody, options, browser) {
				// NOTE: this is how the plugin determines the iframe height,
				// override if you need custom
				return $iframeBody[0].scrollHeight + options.heightOffset;
			};
			jQuery.each(strategyKeys, function(index, value) {
				// use the default strategy for all browsers, can be overridden
				// if desired
			...
			function resizeHeight(iframe) {
				...
				// set the iframe size to minHeight so it'll get smaller on
				// resizes in FF and IE
				if (options.resetToMinHeight && options.resetToMinHeight === true) {
					iframe.style.height = options.minHeight + 'px';
				}
				// get the iframe body height and set inline style to that plus
				// a little
			...
			if ($.browser.webkit || $.browser.opera || $.browser.chrome) {
				...
				$(this).load(function() {
					...
					if (loadCounter === 0) {
						...
						// Reset iframe height to 0 to force new frame size to
						// fit window properly
						// this is only an issue when going from large to small
						// iframe, not executed on page load
						iframe.style.height = options.minHeight + 'px';
					}
