	...
	<form id="mainForm" class="form-horizontal row-main" method="POST">
		...
		<div class="row-left">
			<div class="row-left-title">
				<input type="text" id="txtKeyword" style="border-radius: 0px; width: 100%; border-bottom: 1px solid #bbb; border-top-width: 0px; border-left-width: 0px; border-right-width: 0px" placeholder="${tk:lang('关键字','') }">
			</div>
			<div id="leftList" plugin-scrollbar>
				<c:forEach items="${componentList }" var="component">
					<div class="flowItem" data-type="${component.type }">${component.icon }${tk:lang(component.name,"") }</div>
				</c:forEach>
				<div class="partlist">
		...
		<div class="row-right step-panel">
			<div class="action-group">
				<div>
					<input class="input-large" name="name" type="text" placeholder="${tk:lang('名称','')}" check-type="required" value="${processVo.name }" maxlength="50">
					<input type="hidden" name="uuid" value="${processVo.uuid }">
					<input type="hidden" name="belong" value="itsm">
				</div>
				<div>
					<select name="type" data-searchable="true" data-placeholder="${tk:lang('类型','')}" class="mustinput" plugin-checkselect>
						<c:forEach items="${typeList}" var="type">
							<option value="${type.id }" ${flowVo!=null and type.id == flowVo.type?'selected':''}>${type.name}</option>
						</c:forEach>
					</select>
				</div>
				<input class="input-xlarge" name="description" type="text" placeholder="${tk:lang('说明','')}" value="${flowVo.description }" maxlength="200">
			</div>
			<div id="tdMain">
				<div id="divPaper"></div>
			</div>
			<div id="btnContain">
				<div class="divBtnItem" id="btnAttribute">
					...
					<div>${tk:lang("属性","")}<span id="spnAttributeCount" class="font-icon fz12"> ${processVo.attributeList.size()>0?processVo.attributeList.size():''} </span>
					</div>
				</div>
				<div class="divBtnItem" id="btnForm">
					...
					<div>${tk:lang("表单","")}<span id="spnFormCount" class="font-icon fz12">
			...
			<div class="btn-bar absolute">
				<div class="btn-group">
					<button type="button" class="btn btn-primary" id="btnSaveArrange">${tk:lang("保存","")}</button>
				</div>
			</div>
		</div>
	</form>
